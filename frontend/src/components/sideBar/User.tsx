...
import { hexToRgb } from '../../utils/design';
...
function User({data}: {data:UserType}) {
  ...
  return (
    <div className="w-full h-14 flex items-center gap-4 px-2  cursor-pointer  hover:shadow-lg  duration-[0.1s] " 
        ...
        <h1 className="font-black" style={{color:design?.colors.bgColor}}> {data.fullName}</h1>

        <CircleFadingPlus className="absolute right-8" style={{color:design?.colors.bgColor}} />
    </div>
